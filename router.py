def route_from_scores(scores: dict): 
    """
    Determines the target departments and priority based on GNN confidence scores.
    """
    ...
    # Primary Selection: Select departments above the high confidence threshold (0.35)
    selected = [d for d, s in valid_scores.items() if s > 0.35]
    
    # Fallback Selection: If no strong match found
    if not selected:
        ...
        if sorted_depts:
            ...
            # Select the second highest if its score is also reasonably high (> 0.1)
    ...
    # Priority Assignment Based on max score of selected departments
